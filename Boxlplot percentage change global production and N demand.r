...
# Define plot layout    
plotmat <- matrix(c(1:4), 2, 2, byrow=T)
...
par(mar=c(4,0.3,3,1.5), oma=c(10,6.3,2,6), tck=0.02 , xaxs="i", yaxs="i", las=1)
...
for(v in c("Yield.weight","NUE.weight"))  # v="Yield.weight"
{
    ...
    { 
      ...
      if(t == "N.00")
      {
        ...
        Global.yield <- 3.45
        Global.N.rate <- 49.5
        Global.NUE <- 0.49 * 1000 * Global.yield / Global.N.rate  
        if(v == "Yield.weight")
        {
          # arrows(x0 = 0,             x1 =  p_y.VS.dose$x[which.min(abs(p_y.VS.dose$y - Global.yield))],
          #        y0 = Global.yield , y1 = Global.yield,
          #        length = 0, lwd=2, col="black", lty=2)
          points(Global.N.rate, Global.yield, pch = 4, col='red', cex = 2)
        } else
        {
          # arrows(x0 = 0,           x1 =  p_y.VS.dose$x[which.min(abs(p_y.VS.dose$y - Global.NUE))],
          #        y0 = Global.NUE , y1 = Global.NUE,
          #        length = 0, lwd=2, col="black", lty=2)
          points( Global.N.rate, Global.NUE, pch = 4, col='red', cex = 2)
        }
          # arrows(x0 = Global.N.rate, x1 = Global.N.rate,
          #        y0 = 0 ,            y1 =  p_y.VS.dose$y[which.min(abs(p_y.VS.dose$x - Global.N.rate))],
          #        length = 0, lwd=2, col="black", lty=2)
      }
    }
  ...
  if(i.v==2)  
  {
    text(175,    15.58, c("Base\nline"), cex=1)
    text(255,    15.58, c("RCP\n4.5"),   cex=1)
    text(335,   15.58, c("RCP\n8.5"),   cex=1)
    legend(145, 13.92, legend=rep("",3), pch=unique(my.pch), col=my.col.b,                          lty=1, ncol=3, bty="n", cex=1, pt.cex=2)
    text(75,    12.27, c("Standard\ncultivar"), cex=1)
    legend(145,  9.19, legend=rep("",3), pch=unique(my.pch), col=my.col.b, pt.bg=my.col.f[c(2,4,6)], lty=4, ncol=3, bty="n", cex=1, pt.cex=2)
    text(75,     7.50, c("High yield\npotential"), cex=1)
    text(75,     3.00, c("Recorded\n mean"), cex=1)
    legend(150,  4.50, legend="", pch=4, col="red", bty="n", cex=1, pt.cex=2)
  }
...
mtext("Global average\n grain yield (t /ha)",                side=2, cex=1.5, line=3,     outer=T, adj=0.85, las=0)
mtext("    Global average N use\nefficiency (kg grain /kg N)", side=4, cex=1.5, line=4,   outer=T, adj=0.85, las=0)
...
for(v in unique(var[c(1,3)]))  # v = "Yield"
{
  ...
  if(i.v == 2)
  {
    legend(x=3, y=150, pch=15, col=my.col.f[1], bty="n", cex=1, pt.cex=2, 
           legend="Current N application rate")
    legend(x=3, y=142, pch=15, col=my.col.f[2], bty="n", cex=1, pt.cex=2,
           legend="N application rate to\nmaximize grain yield")
    legend(x=3, y=130, pch=15, col=my.col.f[3], bty="n", cex=1, pt.cex=2,
...
mtext("Relative change in global production\n from high yield potential traits (%)",  side=2, adj=0.1, cex=1.5, las=0, line=3.2, outer = T)
mtext("Relative change in global N uptake\n from high yield potential traits (%)",    side=4, adj=0.1, cex=1.5, las=0, line=4, outer = T)
